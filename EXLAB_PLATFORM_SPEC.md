...
- 901px 이상 데스크톱에서는 화면 높이를 헤더 아래 viewport에 고정하고,
  `설정·세부 설정 / 미리보기`의 2열 프레임과 하단 준비 상태·주 행동의
  위치를 유지한다. 내용이 길면 왼쪽 설정과 세부 설정만 내부 스크롤하며
  미리보기와 주 행동을 아래로 밀거나 페이지 전체 높이를 늘리지 않는다.
- 900px 이하에서는 고정 높이와 내부 프레임 스크롤을 해제하고
  `설정 → 미리보기 → 세부 설정 → 준비 상태·주 행동` 순서의 자연스러운
  단일 열 문서 흐름으로 전환한다.
- 560px 이하에서는 준비 상태와 주 행동을 세로로 쌓고 주 버튼을 가로
  전체 너비로 확장한다. 프레임을 유지하려고 입력, 글자 또는 간격을
  축소하지 않는다.
- 공통 host의 `.exlab-game-instance`가 데스크톱에서 `viewport - header`
  높이를 제공한다. 게임은 프레임 안의 설정 내용과 미리보기만 소유한다.
  breakpoint, 영역 순서, scroll owner와 action bar 위치는 공통
  `SetupWorkspace` 계약이다.
