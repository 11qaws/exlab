...
test("pins the integrated package and both game catalog entries to 1.3.15", async () => {
  ...
  assert.equal(packageJson.version, "1.3.15");
  assert.match(catalogSource, /id:\s*"roulette"[\s\S]*?version:\s*"1\.3\.15"/);
  assert.match(
    catalogSource,
    /id:\s*"showdown"[\s\S]*?version:\s*"1\.3\.15"/,
  );
});
