...
test("pins the integrated package, license and both games to 1.3.45", async () => {
  ...
  assert.equal(packageJson.version, "1.3.45");
  assert.equal(packageJson.license, "MIT");
  assert.match(licenseSource, /^MIT License\r?\n/);
  assert.match(licenseSource, /Copyright \(c\) 2026 11qaws/);
  assert.match(catalogSource, /id:\s*"roulette"[\s\S]*?version:\s*"1\.3\.45"/);
  assert.match(
    catalogSource,
    /id:\s*"showdown"[\s\S]*?version:\s*"1\.3\.45"/,
  );
  assert.match(showdownSource, /SHOWDOWN · VERSION 1\.3\.45/);
  assert.match(readmeSource, /현재 버전은 `1\.3\.45`/);
