...
과거 버전이 자동 생성한 알려진 5·7·8인 제품 기본 명단과 전체 참가자 문자열이
정확히 일치하면 revision과 참가자 ID에 관계없이 새 4인 정본으로 reconcile한다.
변경 후에는 과거 signature와 일치하지 않아 같은 변환을 다시 적용하지 않는다.
이름이 하나라도 추가되거나 순서가 달라진 사용자 명단은 `코코`, `토로리`,
`토로리 코코`를 포함해도 변경하지 않는다. standalone Showdown도 같은 snapshot
읽기·쓰기 경계를 사용한다.
