...
과거 버전이 자동 생성한 5·7·8인 기본 명단과 전체 참가자 문자열이 정확히 일치하고,
유효한 v2라면 revision과 참가자 ID까지 초기 생성 상태일 때만 새 4인 정본으로
reconcile한다. 변경 후에는 과거 signature와 일치하지 않아 같은 변환을 다시
적용하지 않는다. 단, 정확한 구형 short-five 제품 signature
`레또 / 레카 / 세나 / 코코 / 망징`은 저장 revision·ID와 관계없이 이관한다.
그 밖의 사용자 명단은 `코코`, `토로리`, `토로리 코코`를
